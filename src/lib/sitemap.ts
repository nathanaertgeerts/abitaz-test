...
export const sitemap: SitemapSection[] = [
  {
    ...
    nodes: [
      ...
      { label: "Sale", path: "/[locale]/sale", status: "done", needsBackend: true, note: "CategoryTemplate with an on-sale facet." },
  ...
  },
  {
    ...
    nodes: [
      ...
      { label: "Checkout — payment step", path: "/[locale]/checkout/payment", status: "planned", needsBackend: true, note: "Stripe-via-Odoo (D1). 3DS handled by Stripe via Odoo." },
      {
      ...
      { label: "Checkout — pending", path: "/[locale]/checkout/pending", status: "planned", needsBackend: true, note: "Webhook still processing; keep polling; safe fallback page." },
      { label: "Checkout — failed", path: "/[locale]/checkout/failed", status: "planned", needsBackend: true, note: "Retry CTA → /account/orders/[id]/pay; back-to-cart link; Odoo quotation stays alive." },
      { label: "Order confirmation", path: "/[locale]/order-confirmation", status: "done", needsBackend: true, note: "Order #, payment status, delivery, guest→account claim, track CTA, recommendations." },
      {
        label: "Wishlist",
  ...
  },
  {
    ...
    nodes: [
      { label: "Sign in (OTP request)", path: "/[locale]/login", status: "skeleton", needsBackend: true, note: "3 buttons: Inloggen · Particulier · B2B." },
      { label: "OTP code entry", path: "/[locale]/login/otp", status: "skeleton", needsBackend: true, note: "6-digit code, 10 min TTL, 5 attempts." },
      { label: "Signup — Particulier (B2C)", path: "/[locale]/signup/particulier", status: "skeleton", needsBackend: true, note: "Name + email → OTP. res.partner without VAT, public pricelist." },
      { label: "Signup — B2B", path: "/[locale]/signup/b2b", status: "skeleton", needsBackend: true, note: "Company + VAT + email → OTP. 'B2B Pending' pricelist, manual approval in Odoo." },
      { label: "Account overview", path: "/[locale]/account", status: "skeleton", needsBackend: true },
      { label: "Orders", path: "/[locale]/account/orders", status: "skeleton", needsBackend: true },
      { label: "Order detail", path: "/[locale]/account/orders/[id]", status: "planned", needsBackend: true },
      { label: "Order tracking", path: "/[locale]/account/orders/[id]/tracking", status: "planned", needsBackend: true, note: "Owns → Odoo picking/carrier/tracking." },
      { label: "Retry payment", path: "/[locale]/account/orders/[id]/pay", status: "planned", needsBackend: true, note: "Revalidate stock/price; create new Odoo payment.transaction; back into checkout/return polling." },
      { label: "Return request (order)", path: "/[locale]/account/orders/[id]/return", status: "planned", needsBackend: true, note: "MVP: Odoo portal handoff. Own RMA UI is P3." },
      { label: "Invoices", path: "/[locale]/account/invoices", status: "planned", needsBackend: true, note: "Owns → Odoo account.move list." },
      { label: "Invoice detail", path: "/[locale]/account/invoices/[id]", status: "planned", needsBackend: true },
      { label: "Addresses", path: "/[locale]/account/addresses", status: "skeleton", needsBackend: true },
      { label: "Payment methods (account feature)", path: "/[locale]/account/payment-methods", status: "planned", needsBackend: true, note: "Saved Stripe-in-Odoo cards + B2B terms display. NOT to be confused with the public info page `/payment-options`." },
      { label: "Returns (link to Odoo portal)", path: "/[locale]/account/returns", status: "skeleton", needsBackend: true, note: "MVP: deep-link to Odoo portal. Own RMA UI is follow-up." },
      { label: "Preferences", path: "/[locale]/account/preferences", status: "planned", needsBackend: true, note: "Email prefs, language, marketing consent, comms." },
      { label: "Delete account", path: "/[locale]/account/delete-account", status: "planned", needsBackend: true, note: "GDPR. Step-up OTP required. Explain retained legal data." },
      { label: "Guest order lookup", path: "/[locale]/orders/[id]", status: "planned", needsBackend: true, note: "Public, HMAC-token in query (?token=…), 1y TTL. No login required. `/track-order` resolves into this route." },
    ],
  },
  {
    id: "service",
    ...
    nodes: [
      ...
      { label: "Contact", path: "/[locale]/contact", status: "skeleton", needsBackend: true, note: "Form submission needs an endpoint (Odoo lead or mail relay)." },
      ...
      { label: "Track my order", path: "/[locale]/track-order", status: "skeleton", needsBackend: true, note: "Resolves into the guest lookup `/[locale]/orders/[id]?token=…` — not a standalone tracker." },
  ...
  },
  {
    ...
    nodes: [
      ...
      { label: "Apply for trade account", path: "/[locale]/pro/apply", status: "planned", needsBackend: true },
      { label: "Request a quote", path: "/[locale]/pro/quote", status: "planned", needsBackend: true },
      { label: "Project orders", path: "/[locale]/pro/projects", status: "planned", needsBackend: true },
      { label: "Bulk order upload", path: "/[locale]/pro/bulk-order", status: "planned", needsBackend: true, note: "CSV/Excel SKU upload → quote or draft sale.order." },
  ...
  },
  {
    ...
    nodes: [
      ...
      { label: "Blog post", path: "/[locale]/blog/[slug]", status: "cms", needsBackend: true },
    ],
  },
  {
    id: "legal",
  ...
  },
  {
    ...
    nodes: [
      ...
      { label: "Health — public liveness", path: "/api/health", status: "planned", needsBackend: true, note: "Shallow 200/ok; NO dependency detail (don't leak infra)." },
      { label: "Health — internal deep", path: "/api/internal/health", status: "planned", needsBackend: true, note: "Protected; checks Payload/Postgres/Odoo/Stripe; for monitoring only." },
      { label: "Preview routes (Payload draft)", path: "/api/preview/*", status: "planned", needsBackend: true, note: "Protected." },
    ],
  },
];
